...
class DataLoader(object):
    ...
    def batch_unpack_image_sequence(self, image_seq, img_height, img_width, num_source):
        ...
        tgt_image = tf.slice(image_seq,
                             [0, 0, tgt_start_idx, 0],
                             [-1, -1, img_width, -1])
        image_seq = tf.image.resize(image_seq, [expected_height, expected_width])
        image_seq.set_shape([None, expected_height, expected_width, 3])
        # Source frames before the target frame
        print("Input image shape11:", image_seq.shape)

        src_image_1 = tf.slice(image_seq,
                               [0, 0, 0, 0],
                               [-1, -1, int(img_width * (num_source // 2)), -1])
        # Source frames after the target frame
        print("Input image shape22:", image_seq.shape)
        # src_image_2 = tf.slice(image_seq,
        #                        [0, 0, int(tgt_start_idx + img_width), 0],
        #                        [-1, -1, int(img_width * (num_source // 2)), -1])

        src_image_2 = tf.slice(image_seq, begin=[0, 0, 0, 0], size=[-1, -1, img_width, -1])
