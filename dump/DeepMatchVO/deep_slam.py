...
from geo_utils import get_relative_pose, projective_inverse_warp, mat2euler, \
...
class DeepSlam(object):
        ...
        with tf.name_scope("compute_loss"):
            ...
            for s in range(opt.num_scales):
                ...
                for i in range(opt.num_source):
                    ...
                    above_perct_thresh_region = tf.reduce_max(
                        tf.cast(tf.equal(curr_proj_error, perct_thresh), 'float32'), axis=3)
    ...
    def build_pose_test_graph(self, input_uint8):
        ...
        # Forcefully resize both tensors
        target_height = self.img_height  # e.g., 128
        target_width = self.img_width  # e.g., 416
        tgt_image = tf.image.resize(tgt_image, [target_height, target_width])
        src_image_stack = tf.image.resize(src_image_stack, [target_height, target_width])

        # Ensure 3 channels for both
        tgt_image = tf.image.grayscale_to_rgb(tgt_image) if tgt_image.shape[-1] == 1 else tgt_image
        src_image_stack = tf.image.grayscale_to_rgb(src_image_stack) if src_image_stack.shape[
                                                                            -1] == 1 else src_image_stack

        with tf.name_scope("pose_prediction"):
            tgt_image = tf.image.resize(tgt_image, [128, 416])

            # If src_image_stack has multiple images stacked along the channel axis,
            # split them, resize each, then re-stack
            src_images = tf.split(src_image_stack, self.seq_length - 1, axis=3)
            src_images_resized = [tf.image.resize(img, [128, 416]) for img in src_images]
            src_image_stack = tf.concat(src_images_resized, axis=3)
            pred_poses, _ = pose_net(tgt_image, src_image_stack, is_training=False)
            self.pred_poses = pred_poses
