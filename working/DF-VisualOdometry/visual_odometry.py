...
class VisualOdometry:
    ...
    def compute_trajectory(self):
        # print("====>", "preds/preds_" + self.save_path + ".txt")
        ...
        for filename in os.listdir(self.folder_path):
            ...
            # to make a prediction we need consecutive frames, so we skip the first one, and start by comparing the
            # first frame with the second
            ...
            cols, rows, matched_cols, matched_rows = self.flow_ui.get_matches(path_1, path_2, mode="local_top_k",
                                                                              draw=False, dataset=self.dataset)
    ...
    def cam_stuff(self, cam_type):
        ...
        if cam_type == 'drone':
            intrinsic_matrix = np.array(
                [[446.38011635796755, 0, 306.68683920642184], [0, 445.88719476806966, 246.3972559515397], [0, 0, 1]])
        else:
            intrinsic_matrix = np.array(
                [[7.188560000000e+02, 0, 6.071928000000e+02], [0, 7.188560000000e+02, 1.852157000000e+02], [0, 0, 1]])
