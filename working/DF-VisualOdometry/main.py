...
def main(args):
    if args.predict:
        ...
        if args.dataset == 'drone':
            ...
            ground_truth = load_poses_from_txt(
                r"/data/kitti-odom/09/poses.txt")
            preds = load_poses_from_txt(args.input)
            print('preds==>', preds)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--predict', action='store_true',
                        help='Compute a new trajectory if True, else evaluate an already computed trajectory')
    parser.add_argument('-i', '--input',
                        help='Path to the input file, either a folder of images if -t, or a preds text file if not -t')
    parser.add_argument('-o', '--output',
                        help='Name of the output file, it will be saved in either /plots or /preds depending on the flag --test')
