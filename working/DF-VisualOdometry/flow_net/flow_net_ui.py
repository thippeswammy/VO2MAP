        ...
        for y in range(max_y):
            for x in range(max_x):
                ...
                try:
                    ...
                    if p0_[0] >= max_y or p0_[1] >= max_x or p0_[0] < 0 or p0_[1] < 0:
                        warped[:, y, x] = -10000
                    # backward flow in p0_
                    else:
                        warped[:, y, x] = backward[:, p0_[0], p0_[1]]
                except Exception:
                    print(y, x, dx, dy, p0_)
                    exit()
        return warped
    ...
    def get_matches(self, path_1, path_2, dataset, mode='local_top_k', draw=False):
        ...
        tenOne = torch.FloatTensor(
            np.ascontiguousarray(
                np.array(PIL.Image.open(path_1).convert("RGB").resize((640, 192)))[:, :, ::-1]
                .transpose(2, 0, 1)
        ...
        tenTwo = torch.FloatTensor(
            np.ascontiguousarray(
                np.array(PIL.Image.open(path_2).convert("RGB").resize((640, 192)))[:, :, ::-1]
                .transpose(2, 0, 1)
                .astype(np.float32) * (1.0 / 255.0)
            )
        )
        # tenTwo = torch.FloatTensor(np.ascontiguousarray(
        #     np.array(PIL.Image.open(path_2).resize((640, 192)))[:, :, ::-1].transpose(2, 0, 1).astype(np.float32) * (
        #                 1.0 / 255.0)))
        ...
        if mode == 'local_top_k':
            for i in range(N_PATCHES):
                for j in range(N_PATCHES):
                    ...
                    rows_p, cols_p = np.unravel_index(top_i,
                                                      partial_c.shape)  # unraveled relatively to the current patch
