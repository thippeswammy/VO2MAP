    ...
    def process_vo(self):
        ...
        with self._map_lock.r_locked():
            ...
            if vo_ret['n_registered'] == 0:
                ...
                self.edges.append(
                    Edge(self.fid_cur, self.fid_cur + 1, pose=Edge.pose_static, pose_covar=Edge.pose_covar_null,
                         edge_type='none'))
                ...
                for i in range(vo_ret['n_registered']):
                    ...
                    covis = eval_covisibility(vo_ret['depth'], T_tmp, self.K,
                ...
                if self.fid_cur_spakf >= 0:
                    ...
                    covis = eval_covisibility(self.frames[self.fid_cur_spakf].get_scaled_depth(), T_spa2cur, self.K,
                                              self.frames[self.fid_cur_spakf].depth_conf > self.depth_covis_conf_thresh)
        ...
        with self._map_lock.r_locked():
            ...
            for fid in kf_ids:
                ...
                depth = cv2.GaussianBlur(depth,
                                         (self.falign_local_depth_gblur_width, self.falign_local_depth_gblur_width), 0)
                depths.append(depth)
                ...
                if self.use_image_info:
                    ...
                    image = cv2.GaussianBlur(self.images_grayf[fid],
                                             (self.falign_local_image_gblur_width, self.falign_local_image_gblur_width),
                                             0)
                    images.append(image)
            ...
            for i1 in range(len(kf_ids) - 1):
                for i2 in range(i1 + 1, len(kf_ids)):
                    ...
                    self.edges.append(Edge(kf_ids[i1], kf_ids[i2], pose7, falign_ret['poses_covar'][i2],
                                           pose_eval_time_scale=pose_eval_time_scale, edge_type='falign-local'))
        ...
        with self._map_lock.r_locked():
            ...
            for fid in kf_ids:
                ...
                if self.use_image_info:
                    if not self.image_loader_sync(fid):
                        raise 'Image loader not working or files are missing.'
                    image = cv2.GaussianBlur(self.images_grayf[fid],
                                             (self.falign_lc_image_gblur_width, self.falign_lc_image_gblur_width), 0)
                    images.append(image)
            ...
            for i1 in range(len(kf_ids) - 1):
                for i2 in range(i1 + 1, len(kf_ids)):
                    ...
                    pose_eval_time_scale = np.sqrt(f1_scale * f2_scale)
                    self.edges.append(Edge(kf_ids[i1], kf_ids[i2], pose7, falign_ret['poses_covar'][i2],
                                           pose_eval_time_scale=pose_eval_time_scale, edge_type='falign-lc'))
            ...
            if n_kfs_registered == n_kfs_cur:
                ...
                if not self.disable_local_mapping:
                    for f1 in range(max(0, n_kfs_cur - 2 * self.mp_temporal_sigma), n_kfs_cur):
                        for f2 in range(f1 + 1, min(n_kfs_cur, f1 + 2 * self.mp_spatial_sigma)):
                            priority_mat[f1, f2] = max(priority_mat[f1, f2], np.exp(
                                - ((f1 - f2) / self.mp_spatial_sigma) ** 2 - (
                                            (n_kfs_cur - f1) * (n_kfs_cur - f2) / self.mp_temporal_sigma ** 2)))
                ...
                for f1, f2 in self.lc_candidates:
                    for ff1, ff2 in [(f1, f2), (f1 + 1, f2), (f1 - 1, f2), (f1, f2 + 1), (f1, f2 - 1)]:
                        if ff1 >= 0 and ff1 < n_kfs_cur and ff2 >= 0 and ff2 < n_kfs_cur:
                            priority_mat[ff1, ff2] = max(priority_mat[ff1, ff2], np.exp(
                                - ((abs(ff1 - f1) + abs(ff2 - f2)) / self.mp_lc_sigma) ** 2))
                ...
                if Iy.size > 0 and Ix.size > 0:
                    ...
                    for y, x in zip(Iy, Ix):
                        if (y, x) in lc_pairs:
                            new_lc_link_flag = True
                            task = self.falign_thread_pool.apply_async(self.establish_lc_links,
                                                                       ([self.kf_ids[y], self.kf_ids[x]],))
                            falign_task_pool.append(task)
                        else:
                            new_local_link_flag = True
                            task = self.falign_thread_pool.apply_async(self.establish_local_links,
                                                                       ([self.kf_ids[y], self.kf_ids[x]],))
                            falign_task_pool.append(task)
                        link_mask[y, x] = True
                        priority_mat[y, x] = 0
                ...
                if (n_kfs_cur >= next_pgo_kfid) and (new_local_link_flag or new_lc_link_flag):
                    if new_lc_link_flag:
                        ...
                        self.solve_pgo(
                            self.kf_ids[0 if self.pgo_local_kf_winsize > n_kfs_cur else -self.pgo_local_kf_winsize])
                    self._viewer_signal_map_changed = True
                    new_local_link_flag = False
                    new_lc_link_flag = False
