...
def pose_vec2mat(vec, is_kitti_format):
    if is_kitti_format:
        tx, ty, tz, rx, ry, rz = vec
    else:
        rz, ry, rx, tx, ty, tz = vec
    trans = np.array([tx, ty, tz]).reshape((3, 1))
    # Clip rotation angles to avoid invalid inputs
    ry = np.clip(ry, -np.pi + 1e-5, np.pi - 1e-5)
    rot = euler2mat(rz, ry, rx)
...
def dump_pose_seq_TUM(out_file, poses, times, is_kitti_format=False):
    # First frame as the origin
    first_pose = pose_vec2mat(poses[0], is_kitti_format)
    with open(out_file, 'w') as f:
        for p in range(len(times)):
            this_pose = pose_vec2mat(poses[p], is_kitti_format)
