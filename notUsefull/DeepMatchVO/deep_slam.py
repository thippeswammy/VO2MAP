...
class DeepSlam(object):
    ...
    def build_pose_test_graph(self, input_uint8):
        ...
        # Ensure correct shape and RGB channels
        tgt_image = tf.image.resize(tgt_image, [self.img_height, self.img_width])
        src_image_stack = tf.image.resize(src_image_stack, [self.img_height, self.img_width])
        ...
        with tf.name_scope("pose_prediction"):
            src_images = tf.split(src_image_stack, self.num_source, axis=3)
            src_images_resized = [tf.image.resize(img, [self.img_height, self.img_width]) for img in src_images]
